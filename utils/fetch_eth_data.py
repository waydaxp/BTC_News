...
def _judge_signal(df: pd.DataFrame, interval_label="") -> tuple:
    ...
    if rsi < 35 and df['RSI'].iloc[-2] < 30 and close > ma20:
        ...
        signal = "🟢 做多信号"
        reason = "收盘价站上 MA20 且 RSI 偏多 + MA5 向上"
    elif close < ma20 and below_ma20 and 28 < rsi < 57 and close < ma5_val:
        signal = "🔻 做空信号"
        reason = "收盘价低于 MA20 且 RSI 偏空 + MA5 下行"
    elif close > ma20 * 1.02 and rsi > 60:
        signal = "🟢 趋势偏强"
        reason = "价格显著高于 MA20 且 RSI 强势"
        ...
        signal = "🟢 反转信号"
        reason = "RSI 回升 + 均线突破 + 放量 + 多头形态"
    elif abs(close - ma20) / ma20 < 0.005:
        signal = "⏸ 震荡中性"
        reason = "价格围绕 MA20 波动，暂无方向"
