...
def _flatten_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    yfinance 可能返回 MultiIndex 列；将其拉平成单层，并统一首字母大写
    """
    if isinstance(df.columns, pd.MultiIndex):
        df.columns = df.columns.get_level_values(-1)     # 取最内层，如 'close'
...
def get_btc_analysis() -> dict:
    ...
    signal, trend_up = make_signal(df_1h, df_15m, df_4h)   # 顺序：1h → 15m → 4h
